...
- `404.html` — fully self-contained (inline styles, no JS, absolute URLs only) because Cloudflare Pages serves it at arbitrary missing path depths. Its design tokens are duplicated from `styles.css`; update both when the palette changes.
